...
namespace teamproj2
{
    public partial class adminsmanage : System.Web.UI.Page
    {
        ...
        protected void loadAdmins()
        {
            ...
            con.Open();
            Response.Write("<h3>Admin Manage</h3>");
            SqlCommand cmd = new SqlCommand("select * from Admins ORDER BY ID ASC", con);

            if (Request.Form["ausername"] != null)
            {
                string ausername = Request.Form["ausername"];
                string apassword = Request.Form["apassword"];
                ...
                SqlCommand cmd2 = new SqlCommand("insert into Admins (Username,Password) values('" + ausername + "','" + apassword + "')", con);
                cmd2.ExecuteNonQuery();
            }
            if (add2 == 1)
            {
                Response.Write("<form action='adminsmanage.aspx' method='post'>");
                Response.Write("Username: <input type='text' name='ausername'/><br>");
                Response.Write("Password: <input type='password' name='apassword'/><br>");
            ...
            while (rdr.Read())
            {
                ...
                Response.Write("<td>");
                int id4 = (int)rdr[0];
                Response.Write("<a href='adminsmanage.aspx?del=" + id4 + "' class='btn btn-danger'>Delete</a>");
                Response.Write("</td>");
                Response.Write("</tr>");
            }
            con.Close();
